...
#define TAB_TITLE_ACCELEROMETER     L"Bone Angles"
    ...
    , m_hStopStreamEventThread(INVALID_HANDLE_VALUE)
{
 ...
 //   m_tabbedViews.push_back((m_pTiltAngleView));
  ...
  //  m_pSkeletonStream->SetSecondStreamViewer(m_pSecondaryView);
	m_pSkeletonStream->SetStreamViewerAccel(m_pAccelView);
...
bool KinectWindow::Initialize()
{
    ...
    HRESULT hr = m_pNuiSensor->NuiInitialize(NUI_INITIALIZE_FLAG_USES_COLOR
...
void KinectWindow::UpdateStreams()
{
	...
	m_pSkeletonStream->ProcessStream(m_pAccelView);
}
...
void KinectWindow::UpdateTimedStreams()
{
	...
	m_pPrimaryView->ProcessStream(m_pAccelView);
